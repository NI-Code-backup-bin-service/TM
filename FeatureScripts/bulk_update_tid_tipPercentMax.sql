SET @v_tids := '';
SET @v_newValue := '';
SELECT @v_data_element_id := data_element_id FROM data_element WHERE name = 'gratuityMax' AND data_group_id = (SELECT data_group_id FROM data_group WHERE name = 'modules');
