...
BEGIN
  ...
  IF EXISTS(SELECT * FROM approvals a
            WHERE a.profile_id = profile_id
              AND a.data_element_id = data_element_id
              AND approved = 0) THEN
    UPDATE approvals
    SET
      new_value = dataValue,
    ...
    WHERE approvals.profile_id = profile_id
      AND approvals.data_element_id = data_element_id
      AND approvals.approved = 0;
  ELSE
