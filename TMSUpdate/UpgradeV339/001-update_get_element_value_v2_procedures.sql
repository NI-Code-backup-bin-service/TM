...
BEGIN
    declare site_id, chain_id, acquirer_id, is_encrypted, is_password int default 0;
    ...
    IF (chain_id > 0)
        THEN
            SELECT cd.datavalue, IFNULL(cd.is_encrypted,0) as is_encrypted, IFNULL(de.is_password,0)  AS isPassword from chain_data cd inner join data_element de on de.data_element_id=cd.data_element_id WHERE cd.data_element_id = element_id AND cd.profile_id = profile_id   LIMIT 1;
    ELSE
        SELECT pd.datavalue, IFNULL(pd.is_encrypted,0) as is_encrypted, IFNULL(de.is_password, 0) AS isPassword FROM profile_data pd inner join data_element de on de.data_element_id=pd.data_element_id WHERE pd.data_element_id = element_id AND pd.profile_id = profile_id ORDER BY pd.version DESC LIMIT 1;
    END IF;
END IF;
END;