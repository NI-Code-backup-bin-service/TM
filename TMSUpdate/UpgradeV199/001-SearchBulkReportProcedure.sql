--multiline;
CREATE PROCEDURE `search_report_data_batch`( IN siteIds LONGTEXT, IN updatesFrom BIGINT )
BEGIN
...
from (select priority from profile_type where name = "site" order by priority) l
    into @datavalues;
...
select concat_ws(" ",
                 ...
                 @joins,
                 " where e.site_id IN (Select site_id FROM site_data_elements as p WHERE p.site_id IN (", siteIds,
                 "))",
                 "AND ",
