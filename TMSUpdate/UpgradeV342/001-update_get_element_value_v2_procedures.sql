...
BEGIN
    ...
    DECLARE isEncrypted,isPassword int;
    ...
    IF (site_id > 0)
        THEN
            ...
            select r.datavalue, IFNULL(r.is_encrypted, 0) as isEncrypted, IFNULL(r.is_password, 0) as isPassword from results as r LIMIT 1;
    ELSE
    select ifnull(cp.chain_id, 0) into chain_id from profile p inner join profile_type pt on pt.profile_type_id=p.profile_type_id inner join chain_profiles cp on cp.chain_profile_id=p.profile_id where p.profile_id=profile_id and pt.name='chain';
    IF (chain_id > 0)
        THEN
            SELECT cd.datavalue, IFNULL(cd.is_encrypted,0) AS isEncrypted, IFNULL(de.is_password,0)  AS isPassword from chain_data cd inner join data_element de on de.data_element_id=cd.data_element_id WHERE cd.data_element_id = element_id AND cd.profile_id = profile_id   LIMIT 1;
    ELSE
        SELECT pd.datavalue, IFNULL(pd.is_encrypted,0) AS isEncrypted, IFNULL(de.is_password, 0) AS isPassword FROM profile_data pd inner join data_element de on de.data_element_id=pd.data_element_id WHERE pd.data_element_id = element_id AND pd.profile_id = profile_id ORDER BY pd.version DESC LIMIT 1;
    END IF;
END IF;
END;