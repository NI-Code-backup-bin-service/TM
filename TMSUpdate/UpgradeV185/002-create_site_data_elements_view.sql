...
SELECT
    ...
    `de`.`file_max_size` AS `file_max_size`,
    `de`.`file_min_ratio` AS `file_min_ratio`,
    `de`.`file_max_ratio` AS `file_max_ratio`
FROM
    ((((`data_element` `de`
        JOIN `profile_data_group` `pg` ON ((`pg`.`data_group_id` = `de`.`data_group_id`)))
        JOIN `site_profiles` `sp` ON ((`sp`.`profile_id` = `pg`.`profile_id`)))
        LEFT JOIN `data_element_locations_data_element` `delde` ON ((`delde`.`data_element_id` = `de`.`data_element_id`)))
        JOIN `data_element_locations` `del` ON ((`delde`.`location_id` = `del`.`location_id`)))