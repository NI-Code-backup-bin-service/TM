--multiline;
CREATE PROCEDURE `update_eod_auto_time_range`(IN profileId INT, IN hour INT, IN minute INT, IN diff INT)
BEGIN
    ...
    IF (@profileTypeId=4) THEN
        ...
        SET t.auto_time = IF (( @time := ( minute + MOD(RIGHT(t.tid_id, 3), diff) ) ) DIV 60 > 0,
                CONCAT(LPAD(IF (hour=23, 0, hour + 1),2,0), ":",  LPAD(MOD(@time, 60),2,0)),
                CONCAT(LPAD(hour ,2,0), ":", LPAD(@time,2,0)))
