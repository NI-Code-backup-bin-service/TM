...
IF (@merchantIdTest is not null) THEN
	set @message_text = concat('Merchant ID provided already exists: ', @merchantId);
	SIGNAL CUSTOM_EXCEPTION SET MESSAGE_TEXT = @message_text;
    LEAVE this_proc;
END IF;
IF (@acquirerProfileType != "acquirer") THEN
	set @message_text = concat('Acquirer ID provided is not a valid Acquirer: ', @acquirerId);
	SIGNAL CUSTOM_EXCEPTION SET MESSAGE_TEXT = @message_text;
	LEAVE this_proc;
END IF;
IF (@chainProfileType != "chain") THEN
	set @message_text = concat('Chain ID provided is not a valid Chain: ', @chainId);
	SIGNAL CUSTOM_EXCEPTION SET MESSAGE_TEXT = @message_text;
	LEAVE this_proc;
END IF;
call profile_store(-1, 4, @siteName, 1, "Feature Script");
set @profileId = (select profile_id from profile where name = @siteName order by profile_id desc limit 1);
