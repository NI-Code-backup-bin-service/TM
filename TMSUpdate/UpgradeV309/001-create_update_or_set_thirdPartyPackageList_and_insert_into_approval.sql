--multiline;
CREATE PROCEDURE `update_or_set_thirdPartyPackageList_and_insert_into_approval`(IN profileId INT,  IN oldValue varchar(255),IN newValue varchar(255),IN updatedBy varchar(255), tidId INT)
BEGIN
...
INSERT INTO approvals (profile_id,data_element_id, change_type, current_value, new_value, created_at,approved_at, approved, created_by,approved_by, tid_id,acquirer)
				   VALUE
				   (profileId,@dataElementID, 5, oldValue, newValue, NOW(),NOW(), 1, updatedBy,updatedBy, tidId,@acqName);
END