...
BEGIN
    select dg.data_group_id,
           ...
           IF(pd.datavalue = '', null, pd.datavalue),
           de.datatype,
           de.is_allow_empty,
           de.options,
           de.displayname_en,
    ...
    FROM data_group dg
             LEFT JOIN profile_data_group pdg ON pdg.profile_id IN (acquirer_id, chain_id, 1) AND dg.data_group_id = pdg.data_group_id
    ...
    WHERE del.location_name = 'site_configuration';
END;