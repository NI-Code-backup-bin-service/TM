...
BEGIN
    ...
    SELECT p.profile_id
    INTO siteId
    FROM tid_site_profiles
             INNER JOIN profile p ON tid_site_profiles.profile_id = p.profile_id
             INNER JOIN profile_type pt ON p.profile_type_id = pt.profile_type_id
    WHERE tid_id = overrideId
      AND pt.name = 'site';

    SELECT COUNT(*)
    INTO v_nonSiteConfigProfileDataCount
    ...
    WHERE (pd.profile_id = siteId
        AND
           del.location_name != 'site_configuration')
       OR del.location_name = 'fraud';
