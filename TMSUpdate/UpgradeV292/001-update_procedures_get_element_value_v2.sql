...
BEGIN
    ...
    THEN
        ...
        with
            acquirer_p as (
                SELECT distinct sd.datavalue, e.is_encrypted, e.is_password,e.version
    ...
    ELSE
        with
        acquirer_chain_p as (
            SELECT cd.datavalue,cd.is_encrypted, de.is_password from chain_data cd inner join data_element de on de.data_element_id=cd.data_element_id WHERE cd.data_element_id = element_id AND cd.profile_id = profile_id LIMIT 1
        ),
        tid_p as (
            SELECT pd.datavalue, IFNULL(pd.is_encrypted,0) as is_encrypted, IFNULL(de.is_password, 0) AS is_password
        ...
        results as (
            select datavalue, is_encrypted, is_password from acquirer_chain_p
            UNION
            select datavalue, is_encrypted, is_password from tid_p
        )
        select r.datavalue, IFNULL(r.is_encrypted, 0) as is_encrypted, IFNULL(r.is_password, 0) as isPassword from results as r LIMIT 1;
    END IF;
END