--multiline;
CREATE PROCEDURE insert_tid_update_ignore_TPAPK(IN tidId INT, IN targetPackageId INT, IN updateDate DATETIME, IN thirdPartyApk INT)
BEGIN
    SET @tidUpdateId = ((SELECT IFNULL(MAX(tid_update_id), 0) FROM tid_updates WHERE tid_id = tidId)+1);

    IF (thirdPartyApk = 0) THEN
